...
void change_rodata(int32_t fd, Elf64_Shdr sh_table)
{
	char* buff = malloc(sh_table.sh_size);
	assert(lseek(fd, (off_t)sh_table.sh_offset, SEEK_SET) == (off_t)sh_table.sh_offset);
	assert(read(fd, (void *)buff, sh_table.sh_size) == sh_table.sh_size);
	char* ptr = NULL;

	for(int j=0; j<sh_table.sh_size;j++){
		ptr = strstr(&buff[j], "software");

		if (ptr != NULL){
			strncpy(ptr, "hackers!", 8);
			assert(lseek(fd, (off_t)sh_table.sh_offset, SEEK_SET) == (off_t)sh_table.sh_offset);
			assert(write(fd, (void *)buff, sh_table.sh_size) == sh_table.sh_size);
		}
